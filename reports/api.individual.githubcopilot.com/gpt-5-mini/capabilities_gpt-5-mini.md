...
- **Server:** api.individual.githubcopilot.com
- **API type:** OpenAI Completions
- **Context window:** 128,000 tokens (max_prompt_tokens, from the endpoint's rejection message)
- **CORS:** *(CORS does not apply: the probe target is a local script wrapper, not an HTTP endpoint)*
...
**128,000 tokens** — recovered from the endpoint's rejection message.

| Field | Value |
|---|---|
| Tokens | 128,000 |
| Kind | max_prompt_tokens |
| Source | `api_error_message` |
| Evidence | prompt token count of 300008 exceeds the limit of 128000 (Copilot /chat/completions; the local wrapper script swallows the API error body) |
