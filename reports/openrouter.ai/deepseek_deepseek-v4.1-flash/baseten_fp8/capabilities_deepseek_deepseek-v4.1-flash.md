...
- **Provider:** `baseten/fp8` (pinned, fallbacks disabled)
...
| `CORS` | *(no data, please rerun the probing)* |
| `TDEF` | **rejected** (openai-completions: rejected, openai-responses: error, anthropic-messages: n/a) |
...
|---|---|---|---|---|
| `openai-completions` | rejected | — | — | rejects `tool_search` — HTTP 400: Provider returned error 400 {'raw': '{\n\t"error": {\n\t\t"code": "400",\n\t\t"message": "Failed to deserialize the JSON body into the target type: unknown variant `tool_search`, expected... |
| `openai-responses` | error | — | — | HTTP 429: Provider returned error 429 {'provider_name': None} |
